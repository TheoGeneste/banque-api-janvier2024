...
class CompteController {
    ...
    async addCompte(request, result){
        try {
            ...
            result.status(500);
            result.json({error : 'Une erreur est surevnue lors de l\'ajout du compte'});
        }
    }
    ...
    async removeCompte(request, result){
        try {
            ...
            result.status(500);
            result.json({error : 'Une erreur est surevnue lors de la suppression du compte'});
        }
    }
    ...
    async updateCompte(request, result){
        try {
            ...
            result.status(500);
            result.json({error : 'Une erreur est surevnue lors de la modification du compte'}); 
        }
    }
}
